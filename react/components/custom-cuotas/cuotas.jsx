...
const CalculadorCuotas = () => {
  const [paymentMethodGroupActive, setPaymentMethodGroupActive] = useState(
    "creditCardPaymentGroup"
  );
  const [checkedOption, setCheckedOption] = useState(
    defaultValues[paymentMethodGroupActive]
  );
  ...
  if (data) {
    ...
    datos.forEach((dato) => {
      ...
      if (
        reference === dato.itemId[product.productId] &&
    ...
    if (info.length === 0) {
      return (
        <>
          ...
          <div className="sinCuotasInfo">
            Podrás acceder a la información de cuotas cuando elijas tu método de
            pago al finalizar la compra.
          </div>
        </>
      );
    }
          ...
          <div className={styles.paymentContainer}>
            <div className={styles.container}>
              ...
              <div className={`${styles.paymentMethodSelectWrapper}`}>
                <select
                  className={`list pl0 flex flex-column`}
                  onChange={handlePayment}
                  value={paymentMethodGroupActive}
                    ...
                    if (paymentMethod !== "promissoryPaymentGroup") {
                      return (
                        <option
                          ...
                          value={paymentMethod}
                          className={
                            paymentMethod === paymentMethodGroupActive
              ...
              <div className={styles.tarjetasCuotas}>
                <PaymentMethodInfo
                  info={info}
                  paymentMethodGroupActive={paymentMethodGroupActive}
...
const PaymentMethodInfo = ({
  ...
  if (paymentMethodGroupActive === "custom201PaymentGroupPaymentGroup") {
    return (
      <p className="pago">
        Pagá el importe total a través de una transferencia bancaria.
      </p>
    );
  } else if (paymentMethodGroupActive === "custom202PaymentGroupPaymentGroup") {
    return <p>Pagá el importe total en nuestras sucursales.</p>;
  }
          ...
          return (
            ...
            >
              <label className={styles.opcionTarjeta}>
                <img
                  src={
                    paymentOptionsIcons[paymentOption]
                      ? paymentOptionsIcons[paymentOption]
                      : "https://nhorizontesa.vteximg.com.br/arquivos/icono-ejemplo-modal.png"
                  }
                  alt={paymentOption}
                ...
                <input
                  ...
                  name="paymentOption"
                  value={paymentOption}
                  checked={paymentOption === checkedOption && true}
...
const CantidadC = ({ checkedOption, info }) => {
  return console.log("hola mundo this is a test");
};
              ...
              if (PaymentSystemName === checkedOption) {
                return (
                  <tr key={i} className="pa1 pv6">
                    <td>
                      {NumberOfInstallments}{" "}
                      {NumberOfInstallments === 1 ? "cuota" : "cuotas"} de $
                      {Value.toLocaleString()}
                    </td>
                    <td>
                      <strong
                        className={
                          InterestRate === 0 || InterestRate === null
                            ? styles.interestZero
                            : styles.interest
                        }
                      >
                    ...
                    <td>
                      <strong>
                        {" "}
                        ${TotalValuePlusInterestRate.toLocaleString()}
                      </strong>
                    </td>
                  </tr>
                );
